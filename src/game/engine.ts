...
export class GameEngine {
  ...
  private acquireTarget(from: THREE.Vector3, range: number, kind: TowerKind): EnemyEntity | null {
    let best: EnemyEntity | null = null;
    let bestProgress = -1;
    for (const e of this.enemies) {
      if (!e.alive) continue;
      // stealth: only EMP and quantum can see them
  ...
  private fire(t: TowerEntity, target: EnemyEntity, def: typeof TOWERS[TowerKind], stats: TowerLevelStats) {
    ...
    if (t.kind === "quantum") {
      // AoE pulse - damage all enemies in splash radius around tower
      const radius = stats.splashRadius || 7;
      const pulse = this.makePulseMesh(stats.range, def.color);
